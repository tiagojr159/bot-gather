...
import { Game, MoveDirection, Emote } from "@gathertown/gather-game-client";
import { NOMEM } from "dns";
global.WebSocket = require("isomorphic-ws");

const SPACE_ID = "4iqstVgJx00pKpIh\\cwi";
	...
	if (message.messageType === "DM") {
		...
		switch (message.contents.toLowerCase()) {
			...
			case "dance":
				...
				break;
			case "pergunta":
				game.setEmote(Emote.QuestionMark);
				break;
			case "wave":
				game.setEmote(Emote.Wave);
				break;
			case "nome":
				game.setName(NOMEM);
				break;
...
setTimeout(() => {
	...
	game.engine.sendAction({
		$case: "setName",
		setName: {
			name: "CWI | Paulo Vitor",
		},
	});
	game.engine.sendAction({
		$case: "setTextStatus",
		setTextStatus: {
			textStatus: "Eu sou um robô!",
		},
	});
	// game.engine.sendAction({
